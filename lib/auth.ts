...
export const { handlers, auth, signIn, signOut } = NextAuth(async () => {
  // Prefer Google OAuth credentials configured in the admin panel, falling
  // back to environment variables so either approach works.
  const s = await prisma.settings
    .findUnique({ where: { id: "singleton" } })
    .catch(() => null);
  const googleClientId = s?.googleClientId || process.env.GOOGLE_CLIENT_ID;
  const googleClientSecret =
    s?.googleClientSecret || process.env.GOOGLE_CLIENT_SECRET;
  return {
