      ...
      <div className="mx-auto max-w-4xl px-4 py-16">
        ...
        {cartItems.length === 0 ? (
          <div className="card mt-8 p-12 text-center text-ink/60">
            ...
            <Link href="/shop" className="btn-primary mt-4">Browse the shop</Link>
          </div>
        ) : (
          <div className="mt-8 grid gap-8 md:grid-cols-3">
              ...
              {cartItems.map((item) => (
                <div key={item.product.id} className="card flex gap-4 p-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-ink/60">{formatCurrency(item.product.price)}</p>
                    <div className="mt-2 flex items-center gap-3">
                      <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="h-8 w-8 rounded-full border border-secondary">-</button>
                      <span className="w-8 text-center">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="h-8 w-8 rounded-full border border-secondary">+</button>
                      <button onClick={() => removeItem(item.product.id)} className="ml-4 text-sm text-accent hover:underline">Remove</button>
                    </div>
                  </div>
            ...
            <div className="card h-fit p-6">
              ...
              <p className="mt-1 text-xs text-ink/50">Delivery calculated at checkout.</p>
